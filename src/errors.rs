...
impl ResponseError for AuthError{
  fn error_response(&self) -> HttpResponse {
    match self{
      ...
      &AuthError::DuplicateValue(ref message) =>{
  ...
  fn from(error: DBError) -> Self {
    match error{
      DBError::DatabaseError(kind, info)=>{
        let message = info.details().unwrap_or_else(
          || info.message()).to_string();

        match kind {
          DatabaseErrorKind::UniqueViolation =>{
            AuthError::DuplicateValue(message)
          }
          _ => AuthError::GenericError(message)
        }
      },
      _ => AuthError::GenericError(
        String::from("Some database error occured!"),
      )
    }
  }
}
