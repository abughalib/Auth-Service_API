...
use super::{
  ...
  errors::AuthError,
  utils::{is_json_request, get_current_user, is_signed_in,
  set_current_user, to_home, verify
  },
  templates::{Me, SignIn},
};
...
pub async fn me(session: Session, req: HttpRequest)->HttpResponse{
  ...
  match is_json_request(&req) {
    true=>{
      user_result.map_or(HttpResponse::Unauthorized().json("Unauthorized"),
...
pub async fn sign_in(data: web::Json<AuthData>, session: Session, 
  req: HttpRequest, pool: web::Data<Pool>)->Result<HttpResponse, AuthError>{

  match is_signed_in(&session){
    true=>{
      let response = get_current_user(&session).map(|user|
...
fn handle_sign_in(data: AuthData, session: &Session, req: &HttpRequest, 
...
pub async fn sign_in_for_browser(data: web::Form<AuthData>, session: Session, req: HttpRequest,
  pool: web::Data<Pool>) -> Result<HttpResponse, AuthError>{
