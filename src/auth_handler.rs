...
use super::{
    ...
    templates::{Me, SignIn},
    utils::{get_current_user, is_json_request, is_signed_in, set_current_user, to_home, verify},
};
...
pub async fn me(session: Session, req: HttpRequest) -> HttpResponse {
    ...
    match is_json_request(&req) {
        true => user_result.map_or(HttpResponse::Unauthorized().json("Unauthorized"), |user| {
            HttpResponse::Ok().json(user)
        }),
        false => user_result.map_or(
...
pub async fn sign_in_for_browser(
    data: web::Form<AuthData>,
    session: Session,
    req: HttpRequest,
    pool: web::Data<Pool>,
) -> Result<HttpResponse, AuthError> {
