...
use actix_web::{HttpResponse, error::BlockingError, http::header::LOCATION, web};
...
fn create_user(path_id: &str, password: &str, pool: &web::Data<Pool>)->Result<SessionUser, AuthError>{
  ...
  confirmations
    ...
    .map_err(|_db_err|
      AuthError::NotFound(String::from("Confirmation not found")))
      .and_then(|mut result| {
...
pub async fn show_password_form(session: Session, path_id: web::Path<String>,
  pool: web::Data<Pool>) -> Result<HttpResponse, AuthError>{
...
pub async fn create_account_for_browser(path_id: web::Path<String>, data: web::Form<PasswordData>, 
  session: Session, pool: web::Data<Pool>)->Result<HttpResponse, AuthError>{

  let id_str = path_id.into_inner();
  let id_str2 = String::from(id_str.as_str());
  let result = web::block(move || 
  create_user(&id_str, &data.into_inner().password, &pool)).await;
